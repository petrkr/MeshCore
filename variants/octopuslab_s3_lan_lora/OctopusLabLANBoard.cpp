...
bool OctopusLabLANBoard::startOTAUpdate(const char* id, char reply[]) {
  if (_ota_server == nullptr) {
    ...
    _ota_server->on("/", WebRequestMethod::HTTP_GET, [](AsyncWebServerRequest* request) {
      request->send(200, "text/html", home_buf);
    });
    _ota_server->on("/log", WebRequestMethod::HTTP_GET, [](AsyncWebServerRequest* request) {
      request->send(SPIFFS, "/packet_log", "text/plain");
    });

    ElegantOTA.begin(_ota_server);
    _ota_server->begin();
  }
